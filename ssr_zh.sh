...
Download_SSR(){
	cd "/usr/local/"
	wget -N --no-check-certificate "https://github.com/Lorry0822/SS/raw/master/manyuser.zip"
...
Service_SSR(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/Lorry0822/SS/master/ToyoDAdoubi-master/other/ssr_centos -O /etc/init.d/ssr; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/Lorry0822/SS/master/ToyoDAdoubi-master/other/ssr_debian -O /etc/init.d/ssr; then
			echo -e "${Error} ShadowsocksR服务 管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/ssr
